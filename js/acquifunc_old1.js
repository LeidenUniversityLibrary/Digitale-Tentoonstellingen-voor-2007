...
function getPopupValue(p) {
	...
	return document.forms[0].elements[p].options[document.forms[0].elements[p].selectedIndex].value
}
...
function gotoCode(aCode) {
  ...
  if (code != null) {
  	...
  	select2Stage1(NBSList, document.forms[0].elements['o1'],document.forms[0].elements['o2']);
  	if (subcode != null)
...
function _htmlCodeObjectResult(codeObj, itemsFound) {
	...
	return '<A href="javascript:gotoCode(\'' + aCode + '\');">' + aCode + ' ' + anObj.d + '(' + itemsFound + ')</A><BR>';
}
...
function _objectsForKeyword(aStr, depth) {
	...
	for (var i=0;i<NBSList.length;i++) {
		...
		else if ((index = NBSList[i].d.toLowerCase().indexOf(aStr.toLowerCase())) >= 0)
			foundSet[f++] = _objectNBS(NBSList[i].v, NBSList[i], index, aStr.length + index)
		if (depth == 0) continue;
		for (var j=0; j<NBSList[i].l.length;j++) {
			if (NBSList[i].l[j].v == aStr)
				foundSet[f++] = _objectNBS(NBSList[i].l[j].v, NBSList[i].l[j], 0, -1)
			else if ((index = NBSList[i].l[j].d.toLowerCase().indexOf(aStr.toLowerCase())) >= 0)
				foundSet[f++] = _objectNBS(NBSList[i].l[j].v, NBSList[i].l[j], index, aStr.length + index)
		}
	}
	return foundSet;
}
...
function writeHeader(aText, hasBack) {
  ...
  w("<font face=verdana size=2 color=red><b>");
  w(aText);
  w("</b></font>");
  if(hasBack) {
    w("</td><td width=\"15%\">");
    w("<a href=\"?\"><b><font size=1 color=\"#253cb2\">");
    w(backString);
    w("</font></b></a>");
  }
  w("</td></tr></table>");
